...
const doSwitchOneMode = (el, acc_host, sum_host) => {if (false) console.log(el);
	...
	request('https://'+window.location.hostname+port+'/cgi/genc/get_acc_id.pl').then(data => { 
	   if (!data.length) {
		...
		if (!switched) { switched = true; rejoin(); } else {if(!playSomeMusic && !shareSomeScreen){fullscreen=true; chat_shown=1;document.id("logger").click();let re=/video-/gi;let a=el.id.replace(re,"");let v=document.id("video-"+a);if(v && !v.fullscreenElement && !check_iOS()){v.requestFullscreen()}(function(){if (document.id('room-header'))  document.id("room-header").style.display="none";if (normal_mode) document.id("room-backer").style.display="block";if (!small_device) {document.id("room").style.minWidth = "480px";document.id("room").style.marginLeft = "0px";}if(Object.keys(participants).length){for(var key in participants){if(participants[key].name!=a){participants[key].dispose();delete participants[key]}}}}).delay(500)}else{if(playSomeMusic){flashText("PLAYING VIDEO! STOP?")}else{flashText("SHARING SCREEN! STOP?")}}}
	   }
	});	
};
...
function Participant(name, myname, mode, myrole) {
	...
	var beller = document.createElement('div');
	var rtcPeer;
...
///	
	...
	beller.onclick = function(e) {	e.preventDefault(); e.stopPropagation(); var yon = window.confirm('Send microphone alarm to '+name+'?!'); if (yon) sendMicAlarm(); }
	...
	beller.className = 'beller';
	beller.id = 'bell-' + name;
	beller.appendChild(document.createTextNode('O'));
	beller.style.display = i_am_guru ? 'block' : 'none';
...
//console.log('name is', name);
	...
	if (name != myname) {container.appendChild(beller);}
